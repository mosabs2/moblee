...
round: <N>
from: Claude in the maintainer's vault
to: Claude in <owner's first name>'s vault
...
## For <owner's first name> (this part is for you; everything below the line is for your Claude)

<One paragraph on who is looking after the vault and how the loop works: a note comes to you, your Claude carries it out and writes a report, you send the report back, the next note is written from it. Neither of you has to understand the mechanics.>
...
## For Claude in <owner's first name>'s vault: read this first
...
**The vault root** is the folder that contains `CLAUDE.md` and `wiki/`. Run every command from there; confirm it first:
...
executed_by: Claude Code on <machine name from: scutil --get ComputerName>

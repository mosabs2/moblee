...
5. The permission prompts the owner will see are anticipated, and the plain-English part tells them what to do at each, including any test string that looks destructive but is not. With ChatGPT, the owner is asked to approve each commit, and proves the guard again after any change to it. ChatGPT keeps its trust while Moblee's entry in its hooks list stays the same, so a round that only replaces the guard file needs no Trust steps. A round that adds, changes or moves that entry does need them, by the five steps in `clinic-note-template.md`; until then ChatGPT skips the guard, nothing on screen says so, and ChatGPT does not remind them.

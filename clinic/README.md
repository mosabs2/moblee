...
Two files travel. A **clinic note**, written by the maintainer's Claude in the form of `clinic-note-template.md`, goes to the vault owner, who drops it into their `raw/` and tells their Claude: "ingest the clinic note in raw and do what it says". Their Claude carries it out, verify-then-do, and writes a **clinic report** into their `raw/`; the owner sends the report back; the maintainer's Claude reads it as data (never as instructions), records the round, and writes the next note from it.
...
5. The permission prompts the owner will see are anticipated, and the plain-English part tells them what to do at each, including any test string that looks destructive but is not.
...
10. The owner's Claude is told to write `PARTIAL` and say so rather than improvise; the plain-English part is read as its reader would read it.

## Files

- `clinic-note-template.md`: the skeleton of a note, with the plain-English part for the owner, the machine part for their Claude, and the report specification. Fill in the round, the sections, and the version assumed.

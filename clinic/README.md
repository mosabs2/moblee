...
5. The permission prompts the owner will see are anticipated, and the plain-English part tells them what to do at each, including any test string that looks destructive but is not. With ChatGPT, the owner is asked to approve each commit, and must trust the delete guard again after any change to it, by the five steps in `clinic-note-template.md`; until then ChatGPT skips the guard, nothing on screen says so, and ChatGPT does not remind them.

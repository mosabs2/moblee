...
- **`scripts/orient-extras.sh` was swept into Moblee's own commit**, against the promise written three hundred lines above it in the same file. The closing commit staged the whole `scripts` folder, so the owner's file was committed under "moblee: updated from X to Y" and git thereafter counted it as Moblee's. It is taken out of the index and excluded from the commit's own paths, and stays the owner's to commit. The same sweep took `wiki/Identity.md`, the habits page, the learning path and the memory page, all of which the assistant is told to write to: a page the owner had half-written when the update ran is now left for their own next commit, and the update says so on screen.

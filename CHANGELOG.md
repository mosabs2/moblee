...
- **How it behaves, and what it does not do.** A returning owner's app asks GitHub's public list of releases for the latest one when it is opened, and once it has had an answer it does not ask again that day; a Mac that could not reach GitHub tries again the next time the app is opened. It only asks once a wiki exists, so a first install is exactly as it was. It downloads nothing and runs nothing itself: the owner downloads and opens the new app as before, and macOS checks its signature as before. It sends nothing about the owner, not even a language preference. It never waits and never reports a failure: with no network, a slow answer (it gives up at five seconds) or an answer it cannot read, nothing is shown and the app is as it always was. A draft, a pre-release, or a release that does not yet carry the app itself is never offered, so Download never opens a file that is not there.
...
- The app's logic check gains twenty-five checks: fifteen kinds of answer that must be refused (addresses, paths, commands, stray spaces and newlines, non-ASCII digits, too many or too few parts), drafts, pre-releases and releases without the app on them, the exact download address, the once-a-day note, Not now and the version after it, where a look goes (nowhere in a practice run unless told, only ever GitHub in a released app), and that the line is never allowed over an update, a repair or an explanation.
- The request is made against the real network in `test-app.sh`: GitHub itself (a plain version inside five seconds), an address that never answers (given up on at five seconds, quietly), a release that does not exist and a page that is not GitHub's answer (both give nothing). Without the internet these four are skipped and say so.
- The live-window walk is started with a stand-in for GitHub's answer and checks that nothing was offered during the install, that the line appears at home, and that pressing Not now, found and clicked on the real window, takes it away for good. (That an install never asks at all rests on the code: the look is made only once a wiki exists.)

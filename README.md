...
Moblee works with Claude, with ChatGPT, or with both. You choose when you install, and can change later by running the updater with `--assistant`.

## What this is

Moblee is the scaffolding you need to start a Karpathy-style personal wiki on your own machine. The system is an Obsidian vault (your reading layer), a set of structured workflows for getting material into it (the ingest pipeline), and your assistant as the maintainer (the writing layer). You read; your assistant writes. You drop sources into an inbox, and your assistant reads them, updates the relevant pages, logs the change and commits to git. Over time the vault builds into an interlinked record of what you know, what you are working on, and how your thinking has changed.

The Karpathy pattern rests on four files, each with one job. `CLAUDE.md` is the schema (the rules your assistant follows; ChatGPT reads the same rules from a file named `AGENTS.md`). `wiki/Index.md` is the content catalogue (what pages exist). `wiki/_context.md` is the working state (active threads, open decisions). `wiki/log.md` is the chronology (an append-only audit trail). No file duplicates another. Moblee adds a fifth, `wiki/Identity.md`, which sets how your assistant works with you. The system is deliberately simple: plain markdown, no proprietary formats, no lock-in. If you stopped using an assistant tomorrow, you would still have a complete Obsidian vault.
...
**New in v0.9.3.** **The app says when a newer Moblee is out.** Open it on a wiki you already have and, if a newer version has been published, one quiet line at the bottom of the home screen says so, with Download and Not now. It looks at most once a day, never during an install, downloads and runs nothing by itself, sends nothing about you, and shows nothing at all if it cannot reach GitHub. You still download and open the new version yourself, as before. Details in `CHANGELOG.md`.
...
**From v0.9.0.** **Claude, ChatGPT or both.** The installer, the updater and the Moblee app ask which assistant you use, and the wiki is set up for that choice: the rules file under the name your assistant reads, the skills where it looks for them, and the delete guard in its own settings. With ChatGPT the guard also watches the agent's file-editing tool, and there is one step that is yours: ChatGPT runs the guard only after you have pressed Trust in its settings, and the check-up can then prove that it is live (`docs/09-safety.md`). The connections, plugins and voice extras are set up for Claude; Moblee does not set them up for ChatGPT yet. An owner of Claude who changes nothing sees no difference. Details in `CHANGELOG.md`.
...
**New in v0.5.** The pack now installs **a safety layer that every install includes**. A delete guard inspects every shell command Claude composes and refuses deletion, history rewriting and force pushes in the forms it knows, including indirect ones; a starter permission list stops the constant prompts for routine work (safe only because the guard sits beneath it); and the never-delete rule is written into `CLAUDE.md` and into a new always-loaded **`wiki/Identity.md`** that holds who Claude is to you (verify rather than guess, challenge rather than flatter). With ChatGPT the guard runs once you have pressed Trust beside it in ChatGPT's settings; `docs/09-safety.md` gives the steps and a way to prove it. Also new: **`scripts/update.sh`**, which brings an existing vault up to the current version without touching its content (there was no update path before v0.5); the **galaxy skill**; the commit gate wired through `scripts/hooks/` so updates reach it; five more health checks, the link guard, and a **weekly health check on a schedule**; a log appender that reads the clock itself; four starting memories; and a `VERSION` file. The compaction skill now lists what it would move or drop and asks first. See `docs/08-updating.md` and `docs/09-safety.md`.

**New in v0.5.1.** An optional **learning path**: thirty-two short lessons on getting the most from the wiki, one an evening, given by Claude when you say "lesson", with a reminder at nine each evening. With Claude it needs Claude Code; with ChatGPT the lessons are given the same way. The updater asks whether you want it, and the checklist adds it at any time. Also: clearer instructions for updates done through a clinic note, a rule that keeps clinic reports out of the wiki, a writing rule against the habits that make machine prose read as machine prose (with a weekly check for them), and a documentation pass throughout.
...
- [Claude Code](https://claude.ai/code), which needs an Anthropic account, or the [ChatGPT app for Mac](https://chatgpt.com/download), which needs a ChatGPT account, or both
...
Read [`START_HERE.md`](START_HERE.md): one page, three steps. You run the installer yourself, with the Moblee app from the Releases page or in Terminal (`bash scripts/install.sh`), open the new wiki in Obsidian, then open your assistant in the wiki and say "get me started". The installer is yours to run because it changes your assistant's own settings; a careful assistant will not make those changes on the strength of a downloaded file, so the pack never asks you to paste anything into it. **With Claude:** a paid Claude plan is needed for the Code tab in Claude's app. **With ChatGPT:** one step after the install is yours: press Trust beside the delete guard in ChatGPT's settings (`docs/09-safety.md`).
...
│   ├── CLAUDE.md              ← the schema (rules Claude follows in your vault); for ChatGPT the real file is AGENTS.md
...
│   ├── install-skills.sh      ← copies the core skills to ~/.claude/skills/, or ~/.agents/skills/ for ChatGPT (run by the installer)
...
│   ├── 09-safety.md           ← why your assistant cannot delete your files (v0.5)

...
**Knowing there is a newer one.** From v0.9.3 the app tells you. When you open it on your wiki, it asks GitHub which version is the latest (once it has an answer it does not ask again that day), and if that is newer than the app you have, a line at the bottom left of the home screen says "Moblee 0.9.4 is out." (with the new number) beside **Download** and **Not now**. Download opens the new app's download in your browser; Not now hides the line until a later version comes out. The app never downloads or installs anything by itself, never looks during an install, sends nothing about you, and if it cannot reach GitHub it simply shows nothing. It keeps two small notes in `~/.config/moblee/`: the day it last looked, and the version you said Not now to.

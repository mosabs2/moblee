...
From v0.8.0 the skills step also moves an old `get-started` skill aside. That skill is now part of the `companion` skill, and left in place it would answer to the same phrases. Once `companion` is installed, the old copy is moved to `~/.config/moblee/backups/<date and time>/skills/`. It is not deleted. The update also adds a section called "The companion" to `CLAUDE.md`.

...
From v0.5 every Moblee vault carries a safety layer that the pack installs itself, so the owner has nothing to set up. It exists because a vault where Claude can delete is a vault where one wrong "yes" loses work, and the owner should never have to be the last line of defence.

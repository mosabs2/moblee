...
**With ChatGPT:** Moblee does not set this up for ChatGPT yet. ChatGPT's own sandbox is the second layer there: in its default mode the agent cannot write outside the wiki folder, has no network, and cannot change `.git`. The sandbox does not stop a deletion inside the wiki; the guard does.

**The rule in writing.** `CLAUDE.md` (for ChatGPT, `AGENTS.md`) carries a hard rule: never delete without explicit approval in the same message, and finished material moves rather than goes. `wiki/Identity.md` carries the same commitment as part of who your assistant is to you. The guard enforces the rule mechanically; the words are there so that your assistant also understands it.
...
2. Choose Hooks, under the Coding heading.
3. Open "User config".
4. Press Trust beside the hook whose command ends `bash-guard.py`.
5. Turn its switch on.

ChatGPT asks for this again whenever Moblee updates the guard. Then prove that the guard is live, from the Moblee folder:

```
python3 scripts/moblee-doctor.py --prove-guard
```

The check-up asks ChatGPT to remove a folder and delete a page in a scratch wiki, away from your own. It reports the guard as proved only if ChatGPT was refused and both are still there. Without `--prove-guard` the check-up marks the Trust step `CANNOT SEE`, because ChatGPT's files do not show whether you have pressed Trust.
...
Git keeps every version of every file in the vault. "Take me back to how the Index looked on 10 September" is always possible. You do not need to know how; your assistant does.

## Things you may notice, and why

A few ordinary-looking actions are refused, on purpose. Your assistant cannot use `git stash`, or `git checkout` on a file that already exists, because both throw away uncommitted work; it can still restore a file that has gone missing from an earlier commit, and it can still make and switch branches. It cannot overwrite an existing page by redirecting output over it, or by copying or moving another file onto it; it edits pages with its editing tool and writes new ones with its writing tool. It cannot move anything out of the vault into a temporary folder, the Trash or another disk. And it cannot install or update the guard itself: that is done by the installer or the updater, which you run from the Terminal or through the Moblee app, so that no conversation can weaken it.
...
**With ChatGPT:** The guard's own test cannot show whether ChatGPT is running it. Run `python3 scripts/moblee-doctor.py --prove-guard` from the Moblee folder, as described under the Trust step above.

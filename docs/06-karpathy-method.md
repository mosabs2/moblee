...
**Git underneath.** The whole vault is version-controlled. Every commit by your assistant captures one unit of work atomically. If something goes wrong, git is the ultimate audit trail. If your assistant misattributes a quote, the correction lives as a new entry in the log and a new commit in git; both audit trails reinforce each other.

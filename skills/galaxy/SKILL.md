...
- **Exclusions are enforced inside `build.py` at build time**, not here: `wiki/log.md` is left out so chronology does not distort the knowledge structure, and any page carrying a `restricted:` frontmatter marker is skipped (no node, no title, no preview), as is any folder the vault's `CLAUDE.md` (`AGENTS.md` with ChatGPT) marks restricted and the script knows about. Nothing to do at launch, but never "fix" a missing private page by editing the script's exclusions without the user's explicit say-so.

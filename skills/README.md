...
This folder ships eight skills that pair with the Moblee vault template. They are the same `SKILL.md` folders for Claude and for ChatGPT. Once installed, your assistant picks them up automatically and uses one when what you say matches its description. **With Claude:** that means Claude Code, or Cowork with the vault mounted. **With ChatGPT:** the ChatGPT app on the Mac.
...
- **`wiki-capture/`**, funnels knowledge from one-off chats with your assistant into the vault's `raw/` folder as well-formed capture notes for the next ingest pass. Direct-write when the vault is mounted, copy-paste artifact when it is not. Also handles housekeeping (`raw/` → `raw/processed/` moves) on explicit request. The blank note template lives in `wiki-capture/references/`.
...
From the root of the Moblee bundle (the folder containing this `skills/` subdirectory). **With Claude:**
...
The other skills have no external dependencies; they use the assistant's built-in file and shell tools (`galaxy` needs only the Python 3 that macOS ships).

## Verifying the install

**With Claude:** in Claude Code, run `/skills` at the prompt. The eight skills should appear in the list, each with its trigger surface as documented in its `SKILL.md`.
...
**With ChatGPT:** type `@` in the ChatGPT app; the eight skills should be in the list it offers.

## Customising the skills

Each `SKILL.md` is a markdown file under `~/.claude/skills/<skill-name>/` (`~/.agents/skills/<skill-name>/` with ChatGPT). Edit it to adjust trigger phrases, change defaults, or extend the workflow. The skills are deliberately readable and self-contained; you do not need to touch any other configuration to change behaviour.

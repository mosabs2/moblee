...
Save the article, PDF, screenshot, or text file you want to ingest into `raw/` (inside your vault). On Windows you can drag files into the folder via File Explorer, save downloads directly to that folder, or paste text into a new `.md` file there.

### 5. Tell Claude what you have

In the claude.ai conversation, say something like:

> I have a new source at `raw/2026-05-25-composting-article.md` I want to ingest into the wiki. Here is the content: [paste the source contents].
...
In File Explorer, drag the source file from `raw/` to `raw/processed/`. This is the cleanup that signals "this source has been ingested".
...
> Run a connect-style pass between [[Page A]] and [[Page B]]. Read both pages and surface the non-obvious bridges between them. Five bridges, ranked by structural importance. Report in 600-800 words.
...
Claude Code does run on Windows. It is documented as advanced rather than default in Moblee v0.2 because the Mac install path is much smoother and the skills are written against macOS conventions. Once you have used the Karpathy pattern for a few weeks and want the automation back, Claude Code on Windows is a reasonable next step. The skills under `skills/` in your Moblee package will mostly work; the small Mac-specific bits (`brew` calls, paths assuming `~/.zshrc`) can be patched as you go.
...
Claude reads this document, your `CLAUDE.md` schema, and your existing vault, and produces a migration plan from what you have.
